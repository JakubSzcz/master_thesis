...
def set_sub_block(starting_ind: int, samples_to_add: int, org_block: np.ndarray, new_block: np.ndarray) -> int:
    ...
    # go with buffer to the start, index is exceed, firstly add leftovers
        ...
        starting_ind = 0
        org_block[starting_ind:starting_ind + samples_left] = new_block[(samples_to_add - samples_left):]
        starting_ind += samples_left
    else:
...
def generate_blocks_matrix(r_blocks_level: int, block_height: int, coefficients: np.ndarray) -> np.ndarray:
    n_blocks = len(coefficients[r_blocks_level])
...
def wavelet_decompostion(signal: np.ndarray, wavelet_family: str, decomposition_level: int):
    max_decomp = pywt.dwt_max_level(len(signal), wavelet_family)
    assert max_decomp >= decomposition_level, f"Desired wavelet decomposition level is too high. Maximum level is {max_decomp}."
