...
def generate_blocks(signal: np.ndarray, decomposition_level: int, block_height: int,
    ...
    :param return_a: flag to return wavelet decomposition coefficients
    ...
    if return_coeffs:
        return range_blocks, domain_blocks, [b, a[0:k]], wave_coeff_pyramid
    else:
...
R, D, coff_to_be_stored = generate_blocks(X, DECOMP_LEVEL, RANGE_BLOCK_HEIGHT, return_coeffs=False)
...
decoded = [np.random.uniform(0, 1, range_block_size) for i in range(n_range)]
