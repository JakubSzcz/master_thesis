...
def generate_range_domain_blocks(range_block_size: int, n_domains: int, signal, domains_overlaps: bool = True) -> (
    ...
    return range_blocks, domain_blocks, indexes


def encode(ranges: list, domains: list) -> list:
    ...
    print(print(f"Number of unique d used in codding process {len(d_unique)}/{n_domains}"))
    return codded
